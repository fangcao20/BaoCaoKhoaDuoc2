...
const gridXayDungDanhMuc = {
  columnDefs: [
        ...
        { headerName: 'KQTT ID', field: 'kqttID', hide: true },
        ...
        { headerName: 'Số lượng SD', field: 'soLuongSD', filter: 'agNumberColumnFilter',
            cellClass: 'ag-right-aligned-cell', valueFormatter: numberFormatter },
        { headerName: 'Số lượng TT', field: 'soLuong', filter: 'agNumberColumnFilter',
            cellClass: 'ag-right-aligned-cell', valueFormatter: numberFormatter },
        { headerName: 'Giá TT', field: 'donGia', filter: 'agNumberColumnFilter',
            cellClass: 'ag-right-aligned-cell', valueFormatter: numberFormatter },
        { headerName: 'Thành tiền TT', field: 'thanhTien', filter: 'agNumberColumnFilter',
            cellClass: 'ag-right-aligned-cell', valueFormatter: numberFormatter },
        { headerName: 'Số lượng KH', field: 'soLuongKeHoach', filter: 'agNumberColumnFilter',
            cellClass: ['ag-right-aligned-cell', 'completed'], valueFormatter: numberFormatter, editable: true,
        },
        { headerName: 'Giá KH', field: 'giaKeHoach', filter: 'agNumberColumnFilter',
            cellClass: ['ag-right-aligned-cell', 'completed'], valueFormatter: numberFormatter, editable: true,
        },
        { headerName: 'Thành tiền KH', field: 'thanhTienKeHoach', filter: 'agNumberColumnFilter',
            cellClass: ['ag-right-aligned-cell', 'completed'], valueFormatter: numberFormatter,
            valueGetter: params => {
                return params.data.giaKeHoach * params.data.soLuongKeHoach;
            },
         },
        { headerName: 'Nhóm dược lý 1', field: 'nhomDuocLy1', filter: true, minWidth: 250 },
        { headerName: 'Nhóm dược lý 2', field: 'nhomDuocLy2', filter: true, minWidth: 250 },
...
function updateXayDungDanhMuc(xayDungDanhMuc) {
    ...
    $('#selectHoatChatXDDM').on('change', function() {
        ...
        $('#checkNDL2').on('change', function() {
            if (this.checked) {
                $('#checkNHD')[0].checked = true;
                $('#checkNHD').trigger('change');
                $('#selectNhomDuocLyXDDM2').val(nhom_duoc_ly2).trigger('change');
            } else {
        ...
        $('#checkNDL').on('change', function() {
            if (this.checked) {
                $('#checkNHD')[0].checked = true;
                $('#checkNHD').trigger('change');
                $('#checkNDL2')[0].checked = true;
                $('#checkNDL2').trigger('change');
                $('#selectNhomDuocLyXDDM').val(nhom_duoc_ly1).trigger('change');
            } else {
                if ($('#checkNDL2')[0].checked) {
                    $('#checkNDL2').trigger('change');
                } else {
                    if ($('#checkNHD')[0].checked) {
                        $('#checkNHD').trigger('change');
    ...
    $('#selectNhomDuocLyXDDM2').on('change', function() {
        updateData(danhmuc, '#selectNhomDuocLyXDDM2', 18);
    })
    $('#selectNhomHoaDuocXDDM').on('change', function() {
        updateData(danhmuc, '#selectNhomHoaDuocXDDM', 19);
    })
}
...
function updateData(danhmuc, id, index) {
    ...
    'nuocSanXuat', 'nhaThau', 'nhomThau', 'soLuong', 'donGia', 'thanhTien', 'nhomDuocLy1', 'nhomDuocLy2', 'nhomHoaDuoc', 'ABC', 'VEN', 'soLuongSD', 'hoatChatID', 'kqttID']
    ...
    for (row of danhmuc) {
        ...
        if (select_value === "Tất cả") {
            ...
            rowDict['stt'] = stt;
            if (rowDict['kqttID'] in changedValues) {
                rowDict['giaKeHoach'] = changedValues[rowDict['kqttID']]['giaKeHoach'];
                rowDict['soLuongKeHoach'] = changedValues[rowDict['kqttID']]['soLuongKeHoach'];
            } else {
...
function update2tablesXDDM(params) {
    if (params) {
        changedValues[params.data.kqttID] = {
            'soLuongKeHoach': params.colDef.field ==  'soLuongKeHoach'? params.newValue : params.data.soLuongKeHoach,
            'giaKeHoach': params.colDef.field ==  'giaKeHoach' ? params.newValue : params.data.giaKeHoach,
        };
    ...
    gridXayDungDanhMuc.api.forEachNode((rowNode, index) => {
        ...
        switch (row.nhomThau.toLowerCase()) {
            case 'bdg':
                ...
                sumBDGkh += (row.giaKeHoach * row.soLuongKeHoach);
                soLuongBDG += row.soLuong;
                soLuongBDGkh += row.soLuongKeHoach;
                break;
            case 'nhóm 1':
                sumGeneric += row.thanhTien;
                sumGenerickh += (row.giaKeHoach * row.soLuongKeHoach);
                sum1 += row.thanhTien;
                sum1kh += (row.giaKeHoach * row.soLuongKeHoach);
            ...
            case 'nhóm 2':
                ...
                sumGenerickh += (row.giaKeHoach * row.soLuongKeHoach);
                sum2 += row.thanhTien;
                sum2kh += (row.giaKeHoach * row.soLuongKeHoach);
                soLuongGeneric += row.soLuong;
                soLuongGenerickh += row.soLuongKeHoach;
            ...
            case 'nhóm 3':
                ...
                sumGenerickh += (row.giaKeHoach * row.soLuongKeHoach);
                sum3 += row.thanhTien;
                sum3kh += (row.giaKeHoach * row.soLuongKeHoach);
                soLuongGeneric += row.soLuong;
                soLuongGenerickh += row.soLuongKeHoach;
            ...
            case 'nhóm 4':
                ...
                sumGenerickh += (row.giaKeHoach * row.soLuongKeHoach);
                sum4 += row.thanhTien;
                sum4kh += (row.giaKeHoach * row.soLuongKeHoach);
                soLuongGeneric += row.soLuong;
                soLuongGenerickh += row.soLuongKeHoach;
            ...
            case 'nhóm 5':
                ...
                sumGenerickh += (row.giaKeHoach * row.soLuongKeHoach);
                sum5 += row.thanhTien;
                sum5kh += (row.giaKeHoach * row.soLuongKeHoach);
                soLuongGeneric += row.soLuong;
                soLuongGenerickh += row.soLuongKeHoach;
